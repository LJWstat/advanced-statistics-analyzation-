...
수치 미분함수를 구현하시오. 이때 함수의 인자로는 대상 함수 f와 미분값이 계산되는 점인 x, h 값과, method를 설정한다. 이때 h=1e-6을 디폴트 옵션으로 사용하고 method 는 “forward”, “backward”, “central” 중에서 선택할 수 있도록 구현한다. 구현한 함수를 이용해 𝑓(𝑥) = 𝑐𝑜𝑠(𝑥) − 𝑥 함수를 [0, 2𝜋]에서 미분하고, 해석적인 도함수 계산과 비교하여
...
KRR_fit = function(X, y, lambda=0.0001, rho=1) 
{
  ...
  # 모델 객체를 리스트로 만들어 반환 
  model = list(X=X, y=y,alpha=alpha, rho=rho, lambda=lambda)
  class(model)="krr" #  class를 지정하면, 그 객체에 맞는 전용 함수(predict, plot 등)  자동으로 연결됨 & 'krr'은 이름일뿐 
  return(model) 
}
...
KRR_predict = function(model, newdata) {
  ...
  y_pred  = numeric(length(newdata)) # 초기화 
  
  for (i in 1:length(newdata)) {       # 예측할newdata의 점 
     k_vec = numeric(length(X_train))    # 그 점에 대한 커널값 벡터 초기화  (하나의 예측 점에 대한 커널 벡터이니 i,j 아니고 j )
    for (j in 1:length(X_train)) {    # X_train의 점 
      k_vec[j] = gaussian_kernel(newdata[i], X_train[j], rho)
    }
    y_pred[i] = t(k_vec) %*% alpha   # 예측값
  }
  return(y_pred) 
} 
```
...
# KRR_plot = function(X,y,newdata, ...)  
# {
#   fitted_krr = KRR_fit(X,y, lambda = lambda, rho = rho) 
#   y_pred = KRR_predict(fitted_krr, newdata)
#   plot(newdata, y_pred) 
# 
# }
...
KRR_plot = function(X,y,newdata, lambda = 0.01, rho = 0.2, ftrue = NULL, ...) # ftrue도 그냥 같이 써줌 
{
  fitted_krr = KRR_fit(X,y, lambda = lambda, rho = rho) 
  y_pred = KRR_predict(fitted_krr, newdata)
  plot(X, y, xlab = "x", ylab = "y", col = "gray60",
       main = "Kernel Ridge Regression (Gaussian Kernel)", ...) 
  if (!is.null(ftrue)) {
    lines(newdata, ftrue(newdata), col = "skyblue", lwd = 2, lty = 2)
  }
  lines(newdata, y_pred, col = "pink", lwd = 2)
  
  legend("topright",
         legend = c("True f(x)", "KRR prediction", "Samples"),
         col = c("skyblue", "pink", "gray60"),
         lwd = c(2, 2, NA),
         pch = c(NA, NA, 19))
}

# 근데 생각해보면 X, x_grid는 다른데 저렇데 한 그림에 그릴 수 있는거야>?
# 둘 다 같은 구간(-1~1)의 x값이기 때문에 같은 그래프 좌표평면 위에 올려도 아무 문제 없음
...
ftrue = function(x) sin(2*pi*x) + 0.5*cos(4*pi*x)# 실제 함수
y = ftrue(X[,1]) + rnorm(n, sd = 0.1)# 노이즈가 포함된 관측값

x_grid = seq(-1, 1, length.out = 200)# 예측할 x 구간 (newdata)

KRR_plot(X, y, newdata = x_grid, ftrue = ftrue, lambda = 0.01, rho = 0.2)
