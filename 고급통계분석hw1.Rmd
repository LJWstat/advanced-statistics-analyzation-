...
rho = 0.2
lambda = 0.01
model = KRR_fit(X, y, rho, lambda)

# 예측
x_grid = seq(-1, 1, length.out = 200)
y_pred = predict.krr(model, x_grid)

plot(X, y, pch = 19, col = "gray60", main = "Kernel Ridge Regression (Gaussian Kernel)",
     xlab = "x", ylab = "y")
lines(x_grid, ftrue(x_grid), col = "blue", lwd = 2, lty = 2)   # true function
lines(x_grid, y_pred, col = "red", lwd = 2)                    # predicted curve
legend("topright", legend = c("True f(x)", "KRR prediction", "Samples"),
       col = c("blue", "red", "gray60"), lwd = c(2,2,NA), pch = c(NA, NA, 19))
